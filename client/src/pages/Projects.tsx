...
import { FolderKanban, Plus, Search, Calendar, Users, DollarSign } from "lucide-react";
    ...
    <DashboardLayout title="Projects">
      ...
      {!loading && filtered.length === 0 && (
        <div className="flex flex-col items-center justify-center h-48 text-gray-600 font-mono text-xs gap-2">
          ...
          <span>{projects.length === 0 ? "No projects yet." : "No projects match your filter."}</span>
        </div>
      )}
              ...
              <motion.div key={project.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06, duration: 0.4 }}>
                <div className="tech-card p-5 h-full">
                  ...
                  <div className="flex items-center gap-4 text-xs font-mono text-gray-500">
                    {project.budget && <span className="flex items-center gap-1.5"><DollarSign className="w-3 h-3" />{project.budget}</span>}
