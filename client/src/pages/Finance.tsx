    ...
    <DashboardLayout title="Finance">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {loadingKpis ? (
          ...
          <div className="col-span-2 lg:col-span-4 tech-card p-4 text-center text-gray-500">No financial data available</div>
        ) : (
      ...
      <div className="tech-card overflow-hidden">
        ...
        {loadingContracts ? (
          <div className="p-5 text-center text-gray-500">Loading contracts...</div>
        ) : contracts.length === 0 ? (
          <div className="p-5 text-center text-gray-500">No contracts available</div>
        ) : (
              ...
              <tbody>
                {contracts.map((c, i) => (
                  <motion.tr key={c.name || i} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.06 }} className="border-b border-white/5 hover:bg-white/2 transition-colors">
                    ...
                    <td className="px-5 py-3 text-white font-bold text-sm font-mono whitespace-nowrap" style={{ color: c.color || "#0066ff" }}>{c.value}</td>
                    <td className="px-5 py-3">
